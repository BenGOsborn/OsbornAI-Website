...
export default function Payment({ status, payment_id_info }) {
            ...
            return (
                <div className="container">
                    <div className="container">
                        ...
                        <br />
                        <br />
                        <h4 style={{ color: "#039be5" }}>
                            Invalid payment URL!
                        </h4>
                        <h5>
                            Please check the URL and try again, or contact us so
                            we can help to resolve the issue.
                        </h5>
                        <br />
                        <br />
                        <br />
            ...
            return (
                <div className="container">
                    <div className="container">
                        ...
                        <br />$
                        <span id="gtm-amount">
                            {parseFloat(payment_id_info.amount).toFixed(2)}
                        </span>{" "}
                        <span id="gtm-currency">
                            {payment_id_info.currency.toUpperCase()}
                        </span>
                        <br />
                        <br />
                        <StripeCheckout
                            ...
                            amount={parseFloat(
                                payment_id_info.amount * 100
                            ).toFixed(2)}
                            currency={payment_id_info.currency.toUpperCase()}
                            token={(token) => {
                                axios
                                    .post(
                                        "https://osbornai-backend.herokuapp.com/pay",
                                        {
                                            token: JSON.stringify(token),
                                            payment_id: payment_id_info._id,
                                        }
                                    )
                                    .then((res) => {
                                        setPageStatus(2);
    ...
    return (
        <div className="Pay container center" style={{ fontSize: 18 }}>
            <Head>
                <title key="title">
                    Complete The Payment Process - OsbornAI
                </title>
                <meta
                    name="description"
                    content="Complete the payment process so that we can get started working with you!"
                    key="description"
                />
                <meta
                    name="keywords"
                    content="payment, pay, osbornai, transaction, payment id, description, currency, amount, purchase, name"
                    key="keywords"
                />
                ...
                <meta
                    property="og:title"
                    content="Complete The Payment Process - OsbornAI"
                    key="ogTitle"
                />
                <meta
...
export async function getServerSideProps({ req, res }) {
    const payment_id =
        req.__NEXT_INIT_QUERY.slug || req.url.replace("/pay/", "");

    try {
        const res = await axios.post(
            "https://osbornai-backend.herokuapp.com/load_payment_id",
