...
export default function Articles({ sorted_article_data }) {
    ...
    React.useEffect(() => {
        const handleScroll = (event) => {
            if (
                document.body.scrollHeight - window.scrollY < 1600 &&
                loading === false
            ) {
                setPage((prev) => prev + 1);
            }
    ...
    React.useEffect(() => {
        setLoading(true);
        const new_articles = sorted_article_data.slice(
            (page - 1) * page_size,
            page * page_size
        );
    ...
    return (
        <div className="Articles">
            <Head>
                ...
                <meta
                    name="description"
                    content="The full list of our existing articles containing the latest topics, news, and tutorials regarding all things data science!"
                    key="description"
                />
                <meta
                    name="keywords"
                    content="articles, data science, ai, machine learning, news, blog, tutorials, osbornai, posts, blogs, education, guides, artificial intelligence, data"
                    key="keywords"
                />
                ...
                <meta
                    property="og:title"
                    content="Explore Our Articles - OsbornAI"
                    key="ogTitle"
                />
                <meta
            ...
            <div className="container center">
                ...
                <div className="row">
                    {articles.map((article) => {
                        if (
                            articles.length % 2 !== 0 &&
                            article.ref === articles[articles.length - 1].ref
                        ) {
                            return (
                                <div
                                    key={article.ref}
